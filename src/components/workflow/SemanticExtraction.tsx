  ...
  return (
    <div className="w-full max-w-4xl flex flex-col items-center gap-8">
      <GlassPanel className="w-full p-6 text-center">
        <h3 className="text-xs font-mono text-gray-500 mb-4 flex items-center justify-center gap-2">
          <Network className="w-4 h-4" /> SEMANTIC EXTRACTION
        </h3>
        <p className="text-sm text-gray-300 leading-relaxed font-sans max-w-2xl mx-auto">
          {INCIDENT_TEXT}
        </p>
      </GlassPanel>

      <div className="relative w-full h-[300px] border border-gray-800 rounded-2xl bg-black/50 overflow-hidden">
        <svg className="absolute inset-0 w-full h-full">
          {SEMANTIC_EDGES.map((edge, i) => {
        ...
        {SEMANTIC_NODES.map((node, i) => {
          ...
          return (
            <motion.div
              ...
              className={`absolute flex items-center justify-center px-4 py-2 rounded-full border text-xs font-mono whitespace-nowrap transform -translate-x-1/2 -translate-y-1/2 shadow-lg backdrop-blur-md ${
                node.type === 'intent' ? 'bg-purple-900/50 border-purple-500 text-purple-200 shadow-[0_0_15px_rgba(168,85,247,0.4)]' :
