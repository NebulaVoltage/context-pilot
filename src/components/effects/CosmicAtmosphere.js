...
export const CosmicAtmosphere = () => {
    ...
    useEffect(() => {
        ...
        const render = () => {
            ...
            const gradient = ctx.createRadialGradient(width * 0.5 + pointerX, height * 0.35 + pointerY, 40, width * 0.5, height * 0.5, width * 0.75);
            if (isTrustWarn) {
    ...
    return (_jsx("div", { className: "fixed inset-0 pointer-events-none z-0 overflow-hidden select-none bg-[#07080C]", children: _jsx("canvas", { ref: canvasRef, className: "absolute inset-0 w-full h-full" }) }));
};
