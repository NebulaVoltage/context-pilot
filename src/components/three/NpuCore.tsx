...
export const NpuCore: React.FC = () => {
  ...
  useFrame((state, delta) => {
    ...
    if (nodesRef.current) {
      ...
      (nodesRef.current.material as THREE.MeshBasicMaterial).opacity = lerp(
        (nodesRef.current.material as THREE.MeshBasicMaterial).opacity,
        nodeBrightness,
        0.1
      );
    }
    ...
    if (particlesRef.current) {
      particleData.forEach((data, i) => {
        ...
        dummy.position.set(
          ...
          data.yOffset * Math.sin(time + data.phase),
    ...
    if (pulseRef.current) {
      if (simState === 'inference') {
        ...
        (pulseRef.current.material as THREE.MeshBasicMaterial).opacity = (1 - pulseTime) * 0.5;
...
const NpuTelemetry: React.FC = () => {
  ...
  return (
    <div className="flex gap-4 mt-2 text-[10px] text-cyan-300/60 font-mono">
      <div>CONF: {data.conf}</div>
      <div>OPS: {data.ops}</div>
      <div>INT8</div>
    </div>
  );
};
